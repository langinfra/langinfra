...
from langinfra.services.tracing.service import (
    TracingService,
...
def mock_tracers():
    with (
        patch(
            "langinfra.services.tracing.service._get_langsmith_tracer",
            return_value=MockTracer,
        ),
        patch(
            "langinfra.services.tracing.service._get_langwatch_tracer",
            return_value=MockTracer,
        ),
        patch(
            "langinfra.services.tracing.service._get_langfuse_tracer",
            return_value=MockTracer,
        ),
        patch(
            "langinfra.services.tracing.service._get_arize_phoenix_tracer",
            return_value=MockTracer,
        ),
        patch(
            "langinfra.services.tracing.service._get_opik_tracer",
            return_value=MockTracer,
        ),
    ):
        yield
...
async def test_start_tracers_with_exception(tracing_service):
    ...
    with (
        ...
        patch("langinfra.services.tracing.service.logger.debug") as mock_logger,
    ):
...
async def test_trace_worker_with_exception(tracing_service):
    ...
    with patch("langinfra.services.tracing.service.logger.exception") as mock_logger:
