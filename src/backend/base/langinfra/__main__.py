...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(
            package_name, include_prerelease=langinfra_is_pre_release(current_version)
        )
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    info_text = (
        ":star2: GitHub: Star for updates → https://github.com/langinfra/langinfra\n"
    ...
    telemetry_text = (
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("LANGINFRA_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve Langinfra.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_link = f"[bold]🟢 Open Langinfra →[/bold] [link={protocol}://{host}:{port}]{protocol}://{host}:{port}[/link]"
