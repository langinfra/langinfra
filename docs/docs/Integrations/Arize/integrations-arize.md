---
title: Integrate Arize with Langinfra
...
To add tracing to your Langinfra application, add Arize environment variables to your Langinfra application.
...
4. Create a `.env` file in the root of your Langinfra application.
5. Add the `ARIZE_SPACE_ID` and `ARIZE_API_KEY` environment variables to your Langinfra application.
...
```
6. Save the `.env` file.
7. Start your Langinfra application with the values from the `.env` file.
```bash
...
For more information, see the [Arize documentation](https://docs.arize.com/phoenix/tracing/integrations-tracing/langinfra#go-to-arize-phoenix).

## Run a flow and view metrics in Arize

1. In Langinfra, select the [Simple agent](/starter-projects-simple-agent) starter project.
...
Each Langinfra execution generates two traces in Arize.
The `AgentExecutor` trace is the Arize trace of Langchain's `AgentExecutor`. The UUID trace is the trace of the Langinfra components.
