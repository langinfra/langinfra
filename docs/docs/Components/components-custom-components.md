...
By default, Langinfra looks for custom components in the `langinfra/components` directory.

If you're creating custom components in a different location using the [LANGINFRA_COMPONENTS_PATH](/environment-variables#LANGINFRA_COMPONENTS_PATH)
`LANGINFRA_COMPONENTS_PATH` environment variable, components must be organized in a specific directory structure to be properly loaded and displayed in the UI:

```
/your/custom/components/path/    # Base directory (set by LANGINFRA_COMPONENTS_PATH)
...
```
/app/custom_components/          # LANGINFRA_COMPONENTS_PATH
...
This folder structure is required for Langinfra to properly discover and load your custom components. Components placed directly in the base directory will not be loaded.
...
1. In Langinfra, click **+ Custom Component** to add a custom component into the workspace.
