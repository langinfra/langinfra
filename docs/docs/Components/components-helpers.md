...
This component retrieves chat messages from Langinfra tables or external memory.

In this example, the **Message Store** component stores the complete chat history in a local Langinfra table, which the **Message History** component retrieves as context for the LLM to answer each question.

![Message store and history components](/img/component-message-history-message-store.png)

For more information on configuring memory in Langinfra, see [Memory](/memory).
...
This component stores chat messages or text in Langinfra tables or external memory.
