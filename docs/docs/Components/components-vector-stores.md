...
This example uses `./chroma-db` to create a directory relative to where Langinfra is running.
...
The **Local DB** component is Langinfra's enhanced version of Chroma DB.

The component adds a user-friendly interface with two modes (Ingest and Retrieve), automatic collection management, and built-in persistence in Langinfra's cache directory.

Local DB includes **Ingest** and **Retrieve** modes.

The **Ingest** mode works similarly to [ChromaDB](#chroma-db), and persists your database to the Langinfra cache directory. The Langinfra cache directory location is specified in `LANGINFRA_CONFIG_DIR`. For more information, see [Environment variables](/environment-variables).
