...
def update_uv_dep(pyproject_path: str, new_project_name: str) -> None:
    """Update the langinfra-base dependency in pyproject.toml."""
    filepath = BASE_DIR / pyproject_path
    content = filepath.read_text(encoding="utf-8")

    if new_project_name == "langinfra-nightly":
        pattern = re.compile(r"langinfra = \{ workspace = true \}")
        replacement = "langinfra-nightly = { workspace = true }"
    elif new_project_name == "langinfra-base-nightly":
        pattern = re.compile(r"langinfra-base = \{ workspace = true \}")
        replacement = "langinfra-base-nightly = { workspace = true }"
    else:
...
def main() -> None:
    if len(sys.argv) != ARGUMENT_NUMBER:
        msg = "Must specify project name and build type, e.g. langinfra-nightly base"
        raise ValueError(msg)
